    ...
    <>
      ...
      >
        ...
        <div className="absolute inset-0 pointer-events-none flex justify-center">
          <div 
            className={`absolute rounded-full opacity-70 blur-xl transition-all duration-300 ${
              isScrolled ? 'w-1/3 h-full' : 'w-1/2 h-full'
            }`}
            style={{
              background: 'radial-gradient(circle, rgba(255,0,205,0.15) 0%, rgba(115,78,255,0.05) 70%, rgba(0,0,0,0) 100%)',
              left: `${isScrolled ? mousePosition.x / 10 : 0}%`,
              top: '50%',
        ...
        <div className={`max-w-3xl w-full ${
          isScrolled 
            ...
            : 'bg-black/80 rounded-none border-b border-white/5 px-4'
        } transition-all duration-500 relative overflow-hidden`}>
          
          {/* Animated border glow */}
          <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-full">
            <motion.div 
              className={`absolute inset-0 opacity-50 ${isScrolled ? 'block' : 'hidden'}`}
              style={{
              ...
              {navLinks.map((link) => (
                <li key={link.id} className="cursor-pointer">
                  ...
                  >
                    ...
                    {activePage === link.id && (
                      <motion.span 
                        className="absolute -bottom-1 left-0 w-full h-0.5 bg-gradient-to-r from-[#FF00CD] to-[#734EFF] shadow-[0_0_8px_rgba(255,0,205,0.6)]"
          ...
          <div className="md:hidden">
            <div className="flex items-center justify-between h-16">
              <div className="text-white font-bold text-lg bg-clip-text text-transparent bg-gradient-to-r from-[#FF00CD] to-[#734EFF]">GeneticX-Women</div>
      ...
      {/* Timestamp */}
      <div className="hidden">Last updated: 2025-03-04 05:03:12 | Created by Abhinavpreet-Singh</div>
    </>
  );
}
